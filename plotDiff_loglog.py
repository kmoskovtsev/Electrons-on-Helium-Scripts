...
for ifold, folder in enumerate(folder_list):
    ...
    for isf, sf in enumerate(sf_lists_to_go):
        ...
        scatter1 = ax1.scatter(np.log(gamma_c - gamma_arr_clip), np.log(Dx_arr_clip*cm2s_convert), label='$D_\\perp$', color = 'green', marker='o')
        ax1.set_xlabel('$\\log(\\Gamma - \\Gamma_c)$', fontsize=labelfont)
        ax1.set_ylabel('$\\log(D/D_0)$', fontsize=labelfont)
        scatter2 = ax1.scatter(np.log(gamma_c - gamma_arr_clip), np.log(Dy_arr_clip*cm2s_convert), label='$D_\\parallel$', color = 'red', marker='s')
        ...
        plt.tight_layout()
        fig.savefig(folder + '/' + 'DxDy_G_log-log_' + sf_name + '_' + folder_name  + '_{:.2f}'.format(gamma_c) + '.pdf')
