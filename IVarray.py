...
and \mu-p curves. This script uses CustomScatter2D integrator with ripplon and phonon scattering. Script plotIV.py
is used to process the data produced by this code.
"""
...
# Parse args
num_var = 13
A, p, a, repeat_x, repeat_y, dt, regime, subfolder, gamma, Efield, Eaxis, data_steps, coulomb = (-1,)*num_var
...
if A < 0 or p < 0 or a < 0 or repeat_x < 0 or repeat_y < 0 or dt < 0 or regime < 0 or subfolder < 0 or gamma < 0 or\
 Efield < 0 or data_steps < 0 or coulomb < 0 or Eaxis < 0:
...
#accelerated phonon scattering for initial thermalization. Actual values later
scatter.set_tables(w_k, W_inv,  wk_ph*50, F_inv, Y_inv, vmin, vmax)
...
#periodic.disable() #we want to thermalize with that potential to get the right structure

#crystal_state = system.take_snapshot(all=True)
## =======================================================================
# Prepare random (liquid) state, and thermalize from there
print("Initial thermalization (anneal) without drive")
T_disorder = 0.7
...
#Return scattering to proper values
...
#gsd filenames:
data_fname = 'E_{}_{:2.10f}_.gsd'.format(Eaxis, Efield)
sample_fname = 'sample_E_{}_{:2.10f}_.gsd'.format(Eaxis, Efield)
