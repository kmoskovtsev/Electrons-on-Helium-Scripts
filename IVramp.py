...
"""
This script calculates transport at fixed A and p, sweeping through E_drive. It sweeps through E_drive
in one run, without resetting the state between E-points. It is intended to run as one long job. The output data
files have the same naming convention as in IVarray.py, so that the same scripts can be used for processing.
"""
...
folder_path = general_path + 'A{:.6f}/'.format(A)
...
for i, Efield in enumerate(E_list):
    ...
    print('Stabilizing at E = {:.7f}... ({:d} of {:d})'.format(Efield, i+1, NE))
    ...
    #Take snapshots
    fname = 'E{}_{:.7f}_.gsd'.format(Eaxis, Efield)
    gsd_dump = hoomd.dump.gsd(filename=folder_path + fname, period=snap_period, group=hoomd.group.all(), phase=0,\
                                static=['attribute', 'topology'])
    hoomd.run(data_steps)
    gsd_dump.disable()
    
    #Record sample trajectories with higher resolution
    gsd_dump = hoomd.dump.gsd(filename=folder_path + 'sample_' + fname, period=25, group=hoomd.group.all(), phase=0,\
                                static=['attribute', 'topology'])
    hoomd.run(10000)
    gsd_dump.disable()
