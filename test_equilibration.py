...
# Parse args
num_var = 12
A, p, a, repeat_x, repeat_y, dt, regime, subfolder, gamma_min, gamma_max, gamma_N, eqtime = (-1,)*num_var
...
if len(sys.argv) == num_var*2 + 1:
    for i in xrange(1, num_var*2 + 1, 1):
        ...
        if sys.argv[i] == '--eqtime':
            eqtime = int(sys.argv[i+1])
else:
...
N_therm = int(eqtime*np.pi*a**1.5/dt) # n_steps to thermalize ~ 40 periods of oscillation

if regime == 'melt':
    gamma_array = np.linspace(gamma_max, gamma_min, gamms_N)
else:
